...
{
   private static final Logger log = Logger.getLogger(ContainerManagedTransactionInterceptor.class);
   ...
   private ApplicationException getApplicationException(InvocationContext invocation, Throwable t)
   {
      TransactionalEJBManager manager = (TransactionalEJBManager) invocation.getTarget();
      return manager.getApplicationException(t.getClass());
   }

   private int getCurrentTransactionTimeout() throws SystemException
   {
   ...
   private int getTransactionTimeout(InvocationContext invocation)
   {
      TransactionalEJBManager manager = (TransactionalEJBManager) invocation.getTarget();
      return manager.getTransactionTimeout(invocation);
   }

   private static void handleEndTransactionException(Exception e)
   {
   ...
   private void handleInCallerTx(InvocationContext invocation, Throwable t, Transaction tx) throws Exception
   {
      ApplicationException ae = getApplicationException(invocation, t);
      if (ae != null)
      {
   ...
   private void handleExceptionInOurTx(InvocationContext invocation, Throwable t, Transaction tx) throws Exception
   {
      ApplicationException ae = getApplicationException(invocation, t);
      if (ae != null)
      {
   ...
   {
      TransactionalEJBManager manager = (TransactionalEJBManager) invocation.getTarget();
      if(manager.getTransactionManagementType() != TransactionManagementType.CONTAINER)
         return invocation.proceed();
      // TODO: is it safe to assume that only manager.invoke is called?
      Method method = (Method) invocation.getParameters()[1];
      TransactionAttributeType attr = manager.getTransactionAttributeType(method);
      switch(attr)
      {
         case MANDATORY:
   ...
   private Object invokeInCallerTx(InvocationContext invocation, Transaction tx) throws Exception
   {
      try
      {
         return invocation.proceed();
   ...
   private Object invokeInNoTx(InvocationContext invocation) throws Exception
   {
      return invocation.proceed();
   }

   private Object invokeInOurTx(InvocationContext invocation, TransactionManager tm) throws Exception
   {
   ...
   private Object mandatory(InvocationContext invocation) throws Exception
   {
      Transaction tx = tm.getTransaction();
      if (tx == null)
      {
   ...
   private Object never(InvocationContext invocation) throws Exception
   {
   ...
   private Object notSupported(InvocationContext invocation) throws Exception
   {
      Transaction tx = tm.getTransaction();
      if (tx != null)
      {
         ...
         catch (Exception e)
         {
            ...
            if(getApplicationException(invocation, e) != null)
            {
               throw e;
            }
   ...
   private Object required(InvocationContext invocation) throws Exception
   {
      int oldTimeout = getCurrentTransactionTimeout();
      int timeout = getTransactionTimeout(invocation);
   ...
   private Object requiresNew(InvocationContext invocation) throws Exception
   {
      int oldTimeout = getCurrentTransactionTimeout();
      int timeout = getTransactionTimeout(invocation);
    ...
    */
   private void setRollbackOnly(Transaction tx)
   {
      try
      {
         tx.setRollbackOnly();
   ...
   private Object supports(InvocationContext invocation) throws Exception
   {
      Transaction tx = tm.getTransaction();
      if (tx == null)
      {
